...
const request = (
  method: "GET",
  path: string,
  query: OXRQueryParams
): Promise<Response> => {
  const url =
    `https://openexchangerates.org/api` +
    path +
    Object.entries(query).reduce(
      (str, [key, value]) => str + `${str.length ? "&" : "?"}${key}=${value}`,
      ""
    );
...
const getHistoricalRates = async (
  appId: string,
  currency: OXRInternationalCurrencies,
  date: Date = new Date()
): Promise<OXEDollarRates> => {
  const res = await request(
    "GET",
    `/historical/${date.toISOString().slice(0, 10)}.json`,
    {
      app_id: appId,
...
export const createOpenExchangeClient = (openExchangeRatesApiKey: string) => {
  const getTodayRate = async (
    ...
    const res = await getHistoricalRates(
